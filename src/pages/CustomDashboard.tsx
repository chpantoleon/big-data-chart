...
const Dashboard = () => {
  ...
  const fetchData = async (from: Date, to: Date, metadata: Metadata) => {
    ...
    try {
      ...
      if (!queryResultsRaw || !queryResultsCached) {
        return;
      }

      setQueryResults([queryResultsRaw, queryResultsCached]);
    } catch (error) {
      console.error(error);
      if (axios.isCancel(error)) {
        console.log('Request canceled:', error.message);
  ...
  // reset zoom
  useEffect(() => {
    if (!queryResults.length || selectedChart !== null) return;

    const series = Object.values(queryResults[0]!.data);
    series.map((_, index) => {
  ...
  // render error pixels
  useEffect(() => {
    if (!queryResults.length || !measures || selectedChart !== null) return;

    const errors = Object.values(queryResults[1].error);
  ...
  useEffect(() => {
    ...
    const error = queryResults[1].error[selectedChart];
        ...
        <Grid container spacing={2}>
          <Grid size={3}>
            <Card variant="outlined" sx={{ p: 1 }}>
              <Grid container spacing={1}>
                ...
                <Grid size={12}>
                  {responseTime !== 0 && queryResults.length && (
                    <>
                      <Typography variant="overline">Response time</Typography>
                      <Box>
                        <ProgressBar
                          value={((queryResults[1].queryTime * 1000) / responseTime) * 100}
                        />
                        <Box
                          display={'flex'}
                          flexDirection={'row'}
                        ...
                        >
                          ...
                          Query time: {(queryResults[1].queryTime * 1000).toFixed(2)}ms
                          &nbsp;&nbsp;&nbsp;
          ...
          <Grid size={9}>
            {!measures.length ? (
              ...
              <>
                {Object.values(queryResults[1].data).map((_, index) => (
